...
included due to low cell counts or fewer than two levels were: `malform`
(n=15), `pnumsga` (n=0).

``` r
# propose a linear model for the data
mdl_bwt <- lm(bwt ~ babysex + gaweeks + blength, data = tidy_data)
# +  +  + momage
#+  +  +  +  
broom::tidy(mdl_bwt)
```
    ...
    ##   <chr>            <dbl>     <dbl>     <dbl>    <dbl>
    ## 1 (Intercept)    -4324.      98.8     -43.8  0.      
    ## 2 babysexFemale    -18.8     10.2      -1.84 6.57e- 2
    ## 3 gaweeks           27.2      1.72     15.8  6.25e-55
    ## 4 blength          128.       2.00     63.9  0.
...
There is one patient whose predicted value is less than 0, but this
patient is a clear outlier in the data. For the majority of the data
except for the tails, the residuals are dispersed evenly on average
...
cv <- crossv_mc(tidy_data, 100) %>% 
  ...
  mutate(primary_model  = map(train, ~lm(bwt ~ babysex + gaweeks, data = .x)),
